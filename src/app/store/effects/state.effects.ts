...
export class StateEffects {
  getUserTrips$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap((props) =>
        this.apiService
          .getAllTrips(props.user_id)
          .pipe(
            map((response) =>
              StateActions.getUserTripListSuccess({ userTrips: response })
            )
          )
      ),
      catchError((err) => {
  ...
  getTripItineraryItems$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap((props) =>
        this.apiService.getTripItineraryList(props.tripId).pipe(
          map((response) => {
            return StateActions.getTripItineraryItemsSuccess({
              tripItinerary: response,
            });
          })
        )
      ),
      catchError((err) => {
        ...
        StateActions.getTripItineraryItemsFailure({
