...
import {
  ...
  getUserTripList,
} from "../../store/actions/state.actions";
import {
  distinct,
  EMPTY,
  first,
...
export class TripComponent implements OnInit {
  ...
  constructor(
    ...
    this.itemForm = new FormGroup({
      cost: new FormControl(this.itemAddEditDetails.cost, [
        Validators.required,
        Validators.min(0),
      ]),
      currency: new FormControl(this.itemAddEditDetails.currency, [
      ...
      title: new FormControl(this.itemAddEditDetails.title, [
        Validators.required,
  ...
  ngOnInit() {
    ...
    this.tripItinerary$ = this.store.select(selectTripItineraryItems).pipe(
      distinct(),
      map((itinerary) => {
        return itinerary;
      })
    ...
    this.tripList$ = this.store
      .select(selectUserTrips)
      .pipe(
        first(),
        switchMap((tripList) => {
          this.currentTrip = tripList.find(
            (trip) => trip?.trip_id === this.tripId
          );
          return tripList;
        })
      )
  ...
  handleAddItem() {
    ...
    const newItem: IItem = {
      ...
      description:
        this.itemForm?.value?.["description"] ?? "No description provided",
      end_location: { latitude: 0, longitude: 0 },
    ...
    this.apiService
      .addItineraryItem(newItem, this.tripId as string)
      .pipe(
        first(),
